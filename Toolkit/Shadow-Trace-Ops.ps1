...
.DESCRIPTION
    ...
    - Prepare structured Shadow Trace Ops placeholders for XDR, OAuth, session behavior, cloud activity, and DLP collection.
...
$Script:LogoCandidateNames = @(
    "ShadowTraceOpsLogo.png",
    "Shadow-Trace-Ops-Logo.png",
    "Shadow Trace Ops Logo.png",
    "logo.png",
    "Logo.png",
    "ShadowTraceOpsLogo.jpg",
    "ShadowTraceOpsLogo.jpeg"
)
...
function Resolve-ToolLogoPath {
    foreach ($candidate in $Script:LogoCandidateNames) {
        $candidatePath = Join-Path $Script:AssetPath $candidate
...
function Get-LogoHtml {
    $resolvedLogoPath = Resolve-ToolLogoPath
    if ($resolvedLogoPath) {
        $toolLogoUri = Get-ImageDataUri -Path $resolvedLogoPath
        if ($toolLogoUri) {
            return "<img class='tool-logo' src='$toolLogoUri' alt='Shadow Trace Ops Logo' />"
        }
    }

    return "<div class='tool-logo-placeholder'>Shadow<br/>Trace<br/>Ops</div>"
}
...
function Get-RiskCategoryCounts {
    $counts = [ordered]@{
        ...
        XDR            = 0
        Endpoint       = if ($Script:Investigation.EndpointContext) { $Script:Investigation.EndpointContext.Count } else { 0 }
        EmailURL       = 0
        CloudApps      = 0
        Gaps           = if ($Script:Investigation.PotentialGaps) { $Script:Investigation.PotentialGaps.Count } else { 0 }
    }

    if ($Script:Investigation.Alerts) { $counts.XDR += $Script:Investigation.Alerts.Count }
    if ($Script:Investigation.Incidents) { $counts.XDR += $Script:Investigation.Incidents.Count }
    if ($Script:Investigation.EmailContext) { $counts.EmailURL += $Script:Investigation.EmailContext.Count }
    if ($Script:Investigation.UrlClickContext) { $counts.EmailURL += $Script:Investigation.UrlClickContext.Count }
    if ($Script:Investigation.CloudActivity) { $counts.CloudApps += $Script:Investigation.CloudActivity.Count }
    if ($Script:Investigation.CloudAppEvents) { $counts.CloudApps += $Script:Investigation.CloudAppEvents.Count }

    return $counts
}

function Get-RiskChartHtml {
    $counts = Get-RiskCategoryCounts
    ...
    $rows = foreach ($key in $counts.Keys) {
        ...
        $width = [math]::Max(4, [math]::Round(($value / $max) * 100, 0))
        $label = switch ($key) {
            "EmailURL" { "Email / URL" }
        ...
        "<div class='risk-row'><div class='risk-label'>$label</div><div class='risk-bar-shell'><div class='risk-bar' style='width:$width%'></div></div><div class='risk-count'>$value</div></div>"
    }

    return @"
<div class='section risk-chart-section'>
  <div class='section-header-row'>
    <div>
      <h2>Largest Areas of Investigation Activity</h2>
      <p class='section-subtitle'>Counts are based on collected indicators, evidence notes, pivots, and generated findings. Higher counts indicate where analysts may need the most review effort, not confirmed compromise.</p>
    </div>
  </div>
...
function Get-ReportMetricCardsHtml {
    $counts = Get-RiskCategoryCounts
    $authCount = $counts.Authentication
    $riskCount = $counts.Identity
    $cloudCount = $counts.CloudApps
    $xdrCount = $counts.XDR
    $urlCount = $counts.EmailURL
...
<div class='metric-grid'>
  ...
  <div class='metric-card'><div class='metric-icon'>↗</div><div class='metric-value'>$urlCount</div><div class='metric-label'>Email/URL Items</div></div>
...
function Add-LogoToForm {
    ...
    if (-not $resolvedLogoPath) {
        Write-ToolLog "Logo not found. Place ShadowTraceOpsLogo.png in $Script:AssetPath to show it in the UI and reports." "INFO"
        return
    }
    ...
    try {
        ...
        $pictureBox.Image = [System.Drawing.Image]::FromFile($resolvedLogoPath)
        $Form.Controls.Add($pictureBox)
        Write-ToolLog "Logo loaded into UI from $resolvedLogoPath" "SUCCESS"
    }
    catch {
...
function Invoke-CloudSessionGapAssessmentPhase1 {
    ...
    Add-UniqueInvestigationItem -Section "PotentialGaps" -Value "If large data movement is observed without matching DLP, audit, or Defender for Cloud Apps visibility, review data protection coverage for the affected cloud apps and sensitive information types."
...
function Add-Phase1CloudAndGapPlaceholders {
    Write-ToolLog "Adding Shadow Trace Ops Phase 1 cloud/session, OAuth, XDR, and DLP investigation placeholders." "INFO"
    ...
    if ($Script:chkDlp.Checked) {
        ...
        $Script:Investigation.DlpVisibility += "Validate whether large data movement has matching DLP, Defender for Cloud Apps, or audit visibility."
    }
}
...
.tool-logo-placeholder { width:110px; height:110px; display:flex; align-items:center; justify-content:center; text-align:center; border:1px dashed #6b4e8a; border-radius:16px; color:#caa2ff; background:rgba(183,132,255,.08); font-weight:700; letter-spacing:.5px; }
...
function Export-InvestigationReport {
    ...
    try {
        ...
        $riskChartHtml = Get-RiskChartHtml
...
.tool-logo-placeholder {
    ...
    text-align: center;
    border: 1px dashed #6b4e8a;
    border-radius: 16px;
    color: #caa2ff;
    background: rgba(183,132,255,.08);
    font-weight: 700;
    letter-spacing: .5px;
}
.tenant-logo {
...
.metric-card:hover {
    border-color: #b784ff;
    box-shadow: 0 0 22px rgba(183,132,255,.18);
}
.section-subtitle {
    color: #cfc7dc;
    margin-top: -6px;
...
.risk-bar {
    height: 100%;
    background: linear-gradient(90deg, #7c4dff, #caa2ff);
    border-radius: 999px;
    box-shadow: 0 0 14px rgba(183,132,255,.35);
}
.risk-count {
    text-align: right;
    color: #ffffff;
