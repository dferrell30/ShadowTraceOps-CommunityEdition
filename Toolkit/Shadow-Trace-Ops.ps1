...
function Convert-SnapshotFindingsToHtml {
...
function Convert-SnapshotReadinessToHtml {
    $rows = @()
...
function Convert-SnapshotNotableTableToHtml {
...
function Export-ExecutiveSnapshotReport {
...
if (-not $Script:Investigation) { return $null }
    ...
    try {
        ...
        $safeName = $upn.Replace("@", "_").Replace(".", "_").Replace("\", "_").Replace("/", "_")
        $timestamp = Get-Date -Format "yyyyMMdd_HHmmss"
        $snapshotFile = Join-Path $Script:ReportPath "ShadowTraceOps-ExecutiveSnapshot-$safeName-$timestamp.html"

        $logoHtml = Get-LogoHtml
        $readinessHtml = Convert-SnapshotReadinessToHtml
        $findingsHtml = Convert-SnapshotFindingsToHtml
        $notableTableHtml = Convert-SnapshotNotableTableToHtml

        $authCount = Get-SnapshotCount -Name "Authentication"
        $riskCount = Get-SnapshotCount -Name "IdentityRisk"
        $cloudCount = Get-SnapshotCount -Name "Cloud"
        $xdrCount = Get-SnapshotCount -Name "Xdr"
        $sourceCount = Get-SnapshotCount -Name "Source"
        $urlCount = Get-SnapshotCount -Name "Url"
        ...
        $lookback = $Script:Investigation.AuthLookbackDays
        $version = "$($Script:Investigation.ToolkitPhase)"
        $phase = "$($Script:Investigation.ToolkitPhase)"
...
<head>
<meta charset="UTF-8">
<title>Shadow Trace Ops Executive Snapshot</title>
<style>
body {
...
.snapshot-shell {
    ...
    padding: 28px;
    background:
        radial-gradient(circle at 85% 8%, rgba(151,75,255,.18), transparent 30%),
        radial-gradient(circle at 15% 20%, rgba(104,43,145,.20), transparent 32%),
        #05070d;
}
.snapshot-card {
    border: 1px solid #5c3f7a;
    border-radius: 18px;
    background: linear-gradient(180deg, rgba(15,17,28,.96), rgba(5,7,13,.98));
    box-shadow: 0 0 38px rgba(124,77,255,.18);
    overflow: hidden;
}
.hero {
...
.brand-wordmark span { color: #a855f7; }
.two-col {
    display: grid;
...
.panel {
    border: 1px solid #4b315f;
    border-radius: 12px;
    background: rgba(9,12,24,.80);
    padding: 18px;
}
.panel h2, .section h2 {
    color: #c26bff;
...
.readiness-table th, .table-clean th {
    color: #ffffff;
    background: rgba(255,255,255,.04);
}
.available { color: #5ee07a; font-weight: 800; }
.limited { color: #ffd166; font-weight: 800; }
.metrics {
    display: grid;
...
.metric {
    border: 1px solid #5c3f7a;
    border-radius: 12px;
    background: linear-gradient(180deg, rgba(55,23,80,.45), rgba(11,13,24,.92));
    text-align: center;
    padding: 22px 10px;
}
.metric-icon {
...
<div class="snapshot-shell">
  <div class="snapshot-card">
    <div class="hero">
      <div>
        <h1>SHADOW TRACE <span>OPS</span> REPORT</h1>
        <div class="hero-subtitle">Investigation Toolkit for Microsoft 365 and XDR</div>
      </div>
      <div class="logo-area">
        $logoHtml
        <div class="brand-wordmark">SHADOW<br/>TRACE <span>OPS</span></div>
      </div>
    </div>
    ...
    <div class="two-col">
      <div class="panel">
        ...
        <div class="info-row"><div class="info-label">Generated:</div><div class="info-value">$generated</div></div>
        <div class="info-row"><div class="info-label">Investigator:</div><div class="info-value">$env:USERNAME</div></div>
        <div class="info-row"><div class="info-label">Target Identity:</div><div class="info-value">$upn</div></div>
        <div class="info-row"><div class="info-label">Time Range:</div><div class="info-value">$lookback day lookback</div></div>
        <div class="info-row"><div class="info-label">Tenant ID:</div><div class="info-value">$tenantId</div></div>
        <div class="info-row"><div class="info-label">Tool Version:</div><div class="info-value">$version</div></div>
      </div>

      <div class="panel">
        <h2>Environment & Telemetry Awareness</h2>
        $readinessHtml
      </div>
    </div>

    <div class="metrics">
      <div class="metric"><div class="metric-icon">USER</div><div class="metric-number">$authCount</div><div class="metric-label">User Sign-Ins</div></div>
      <div class="metric"><div class="metric-icon">RISK</div><div class="metric-number">$riskCount</div><div class="metric-label">Risky Sign-Ins</div></div>
      <div class="metric"><div class="metric-icon">CLOUD</div><div class="metric-number">$cloudCount</div><div class="metric-label">Cloud App Events</div></div>
      <div class="metric"><div class="metric-icon">XDR</div><div class="metric-number">$xdrCount</div><div class="metric-label">XDR Alerts</div></div>
      <div class="metric"><div class="metric-icon">LOGS</div><div class="metric-number">$sourceCount</div><div class="metric-label">Source Notes</div></div>
      <div class="metric"><div class="metric-icon">URL</div><div class="metric-number">$urlCount</div><div class="metric-label">Clicked URLs</div></div>
    </div>

    <div class="section">
      <h2>Notable Investigation Events</h2>
      $notableTableHtml
      <div class="note">Note: Showing top notable items. Use the dashboard and detailed workflow report for full context.</div>
    </div>
    ...
    <div class="value-row">
      <div class="value-cell"><div class="value-icon">E2E</div><div><div class="value-title">End-to-End Visibility</div><div class="value-sub">From sign-in to action.</div></div></div>
      <div class="value-cell"><div class="value-icon">COR</div><div><div class="value-title">Correlate & Detect</div><div class="value-sub">Connect the dots.</div></div></div>
      <div class="value-cell"><div class="value-icon">HUNT</div><div><div class="value-title">Hunt & Respond</div><div class="value-sub">Find threats others miss.</div></div></div>
    </div>

    <div class="footer">
      <span><strong>Shadow Trace Ops</strong> - Investigate. Correlate. Protect.</span>
      <span>Report generated by Shadow Trace Ops $phase</span>
    </div>
  </div>
</div>
</body>
...
"@
        ...
        $html | Out-File -FilePath $snapshotFile -Encoding UTF8
        Test-ShadowTraceHtmlReportFile -Path $snapshotFile -ReportName "Executive snapshot report" | Out-Null
        $Script:CurrentSnapshotReport = $snapshotFile
        $Script:CurrentSnapshotReport = $snapshotFile
        $Script:CurrentSnapshotReport = $snapshotFile
        Write-ToolLog "Executive snapshot report exported: $snapshotFile" "SUCCESS"
        return $snapshotFile
    }
    catch {
        Write-ToolLog "Executive snapshot report export failed: $($_.Exception.Message)" "ERROR"
        Write-ToolLog $_.ScriptStackTrace "ERROR"
        return $null
    }
}
...
if (-not $Script:Investigation) {
    ...
    try {
        ...
        $snapshotReport = Export-ExecutiveSnapshotReport
...
function Open-ExecutiveSnapshotReport {
    if ($Script:CurrentSnapshotReport -and (Test-Path $Script:CurrentSnapshotReport)) {
        Start-Process $Script:CurrentSnapshotReport
        Write-ToolLog "Opened Executive Snapshot report: $Script:CurrentSnapshotReport" "INFO"
        return
    }

    $latestSnapshot = Get-ChildItem -Path $Script:ReportPath -Filter "ShadowTraceOps-ExecutiveSnapshot-*.html" -ErrorAction SilentlyContinue |
        Sort-Object LastWriteTime -Descending |
        Select-Object -First 1

    if ($latestSnapshot) {
        $Script:CurrentSnapshotReport = $latestSnapshot.FullName
        Start-Process $latestSnapshot.FullName
        Write-ToolLog "Opened latest Executive Snapshot report: $($latestSnapshot.FullName)" "INFO"
        return
    }

    [System.Windows.Forms.MessageBox]::Show(
        "No Executive Snapshot report was found. Run an investigation and click Export HTML Report first.",
        "Executive Snapshot Not Found",
        "OK",
        "Information"
    ) | Out-Null
}
...
$lblSnapshotNote = New-Object System.Windows.Forms.Label
$lblSnapshotNote.Text = "Executive Snapshot: created by Export HTML Report"
$lblSnapshotNote.Location = New-Object System.Drawing.Point(570, 145)
$lblSnapshotNote.Size = New-Object System.Drawing.Size(420, 20)
$lblSnapshotNote.ForeColor = $Script:Theme.MutedFore
$form.Controls.Add($lblSnapshotNote)
...
$btnOpenSnapshot = New-Button -Text "Open Snapshot" -X 920 -Y 315 -OnClick { Open-ExecutiveSnapshotReport } -W 160 -H 34
$btnOpenSnapshot.BackColor = [System.Drawing.Color]::FromArgb(18,18,24)
$btnOpenSnapshot.ForeColor = [System.Drawing.Color]::White
$btnOpenSnapshot.FlatAppearance.BorderColor = [System.Drawing.Color]::Black
$btnOpenSnapshot.FlatAppearance.BorderSize = 2
